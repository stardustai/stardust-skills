...
项目的 Spec、Design 和 Plan 采用 Superpowers 路径。以下完整目录是新项目默认模板，
也是已有项目可选择迁移到的标准结构：
...
模板表达的是内容职责，不要求已有项目仅因目录不同而搬迁。若既有结构清晰，总控 Skill
必须让用户二选一：A. 改成标准结构；B. 就地适配结构。选择 B 时，`PROJECT.yaml` 映射
每项职责的真实路径，README 说明组织形态并列出所有文档导航；缺失内容在最合适的既有
位置补齐，不创建重复事实源。两种组织方式执行相同的内容、测试、Eval、证据和运行门禁。
业务项目的 QA 规范化 Spec、测试设计和测试用例也属于映射职责。所有映射必须使用项目
相对路径。文件存在或篇幅足够不能证明内容齐全；独立 Review Agent 必须把每项必需主题和
文档中的唯一完整行定位及其非空章节 Hash 绑定到当前文件 SHA-256，机器校验通过后才算
初始化完成。JSON 身份字段不能自行证明独立性，总控必须保留真实的 Review Agent 派发与
运行轨迹供最终审查。
...
- `PROJECT.yaml` 是机器可读状态，记录 Remote URL、当前 Spec/Design/Plan、风险、
  Owner、文档组织模式和真实路径、标准命令、当前任务、最后稳定 Commit、PR 和部署策略。
...
2. 按内容职责盘点已有文档和缺失内容；
3. 目录不同时让用户选择改成标准结构，或就地适配结构；
4. 按选择补齐 README、PROJECT.yaml、文档路径映射和全部适用内容；
5. 由独立 Reviewer 按路径、SHA-256、必需主题和精确定位验证内容齐全；
6. 确认 Remote Repository URL、默认分支和基线 Commit；
7. 建立当前测试、Eval 和运行基线；
8. 审计 Spec 与现有实现差异；
9. 生成技术债清单和架构修复方案；
10. 让用户在“全量修复技术债”和“最小干净修复”之间选择；
11. 完成修复、验证和独立 Commit；
12. 建立新的初始化基线后再开发功能。

文档组织选择与技术债修复选择分别记录：前者决定资料放在哪里，后者决定哪些结构性
问题必须修复。
...
│   ├── documentation-content-standard.md
...
│   ├── schemas/
│   │   ├── project.schema.json
│   │   ├── documentation-review.schema.json
