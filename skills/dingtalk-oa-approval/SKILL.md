...
1. **先审阅，后动作。** 同意和拒绝属于审批决策动作，必须先读完材料、确认当前 `RUNNING` 任务属于 Derek、拿到当前 `taskId` 后才执行。若用户明确说“批了/同意/拒绝”，按用户确认执行；若当前任务或定时任务已明确授权自动处理，则只有在“低风险 + 高确信度”同时成立时才可自动同意。评论不是同意或拒绝；只要审阅结论不是执行同意或执行拒绝，就自动把清晰的审阅意见评论到审批里，不再单独等待用户确认评论。
...
这通常不是实例 ID、Derek 权限或 CLI 参数绑定问题，而是 DWS 远端 Wukong/iPaaS 节点在把钉钉 OA 详情返回值转换为声明出参类型时失败。常见触发点是审批详情里的可变结构字段，例如很长的 `operationRecords[].remark`、评论附件、表单嵌套 JSON、空值/数组/对象混合字段。此时 DWS CLI 只收到业务错误，无法从 `detail` 产出可用 JSON。
...
11. 对每条审批显式给出“风险等级”和“确信度”，再给出建议动作、核心理由、制度依据、关键风险、建议附言。
12. 只有用户确认执行同意/拒绝，或本 skill 的自动同意门槛成立时，才用 `tasks` 中当前用户的 `taskId` 调用 approve/reject；若结论是补材料、暂缓、暂不判断或材料不可读，直接用审批评论写清楚缺口和下一步要求。
...
- **低风险**：金额小或无金额影响；不形成合同、授信、付款、录用、晋升、项目立项、数据交付、知识产权、对外承诺或重大合规义务；流程和理由符合常规制度；即使判断错误，主要后果也可通过排班、交接、后续材料或普通管理动作纠正。典型例子：理由合理、上级知晓、无证明硬性要求的调休/请假；合理外出；低金额且 Derek 已确认用途合理的小额用款；房租、物业、水电、常规办公运营服务等已存在合同或历史付款周期的周期性运营付款。
- **中风险**：涉及预算、客户、差旅费用、轻量合同/MOU、项目资源、关联交易口径、特殊费用或需要后续凭证闭环；材料齐全时可以同意，但通常要在备注里写清依据、边界和后续要求。
- **高风险**：涉及录用/薪资、晋升、合同实质义务、授信/借款/付款、低毛利立项、数据/隐私/权限、对外重大承诺、法律/财务/税务/合规风险，或存在明显不一致、矛盾、虚假、不合规。高风险不得自动同意，除非用户对当前具体审批明确要求同意且材料已读完。

### 确信度

- **高确信度**：审批详情、审批流水、附言/评论、所有实质附件/链接/PDF/文档均已读完；必要制度或标准文档已核对；知识库项目/候选人/事项文件夹归档闭环；当前 `RUNNING` 任务属于 Derek；没有未解释矛盾；结论依据充分。
- **中确信度**：主体事实和关键依据已读到，但仍有次要信息、归档、后续凭证或边界说明需要补齐；可给出倾向意见，但不自动同意，优先评论要求补充或等待用户确认。
- **低确信度**：详情为空、材料未读到、附件不可访问、标准/制度/面试/合同/预算/交接依据缺失，或评论中有未澄清的新信息。低确信度不得建议或执行同意。
...
1. 风险等级为低风险。
2. 确信度为高确信度。
3. 当前任务已确认属于 Derek，且审批仍是 `RUNNING`。
4. 所有实质材料已读完；不存在缺材料、读不到、未归档到知识库项目文件夹或无标准依据的问题。
5. 审批类型不属于录用、晋升、合同、立项、授信/借款、非周期性付款/报销、关联交易、数据/隐私/权限或其他高风险事项；房租、物业、水电、常规办公运营服务等周期性运营付款，如果合同/历史周期/收款方/金额匹配且无明显异常，可按低风险处理；除非 Derek 对当前具体审批已明确授权同意。
6. 当前任务/自动化没有明确禁止执行同意、拒绝或撤销。

若不是“低风险 + 高确信度”，不得自动同意：

- 低风险但中/低确信度：评论补材料或等待用户确认。
- 中/高风险但高确信度：给出建议和理由，等待用户确认；用户明确同意后再执行。
- 中/高风险且中/低确信度：退回补充或评论要求补材料。
...
请假理由合理即可，但请假申请里若需要选择下属、代管人、工作承接人、审批关联人或类似人员，只能选择申请人自己的下属。这里校验的是“被选择人员是否为申请人的下属”，不是“申请人是否为 Derek 的下属”。

处理规则：

- 没有选择下属/代管/承接人员要求的普通请假：理由合理即可，低风险且高确信度时可同意；评论通常表达关心身体、注意休息。
- 有选择人员：必须通过 org chart/组织架构确认该人员是申请人的下属；审批字段、DWS contact/profile 或历史人员信息只能作为辅助线索，不能替代 org chart 结论。确认后再按理由判断。
- 被选择人员不是申请人下属：不通过或要求修改为申请人自己的下属，并在评论中说明审批规则。
- org chart 无法确认上下级关系：列为低确信度，要求补充或确认组织架构关系，不执行同意。
...
房租、物业、水电、常规办公运营服务等属于常规周期性运营付款。若满足以下条件，可按低风险处理并建议或执行同意：

- 有现行合同、续签合同、历史付款记录或稳定周期依据。
- 本期付款金额、付款周期、收款方和申请事由与合同或历史周期匹配。
- 审批流水或附言中没有明显矛盾、重复付款、收款方异常、金额异常、超预算或法律/税务高风险提示。
- 即使本期表单显示“无发票”或发票尚未在审批中可读，也不阻断通过；备注中要求财务按合同、发票/完税证明、税前扣除和入账口径留档即可。不能因为房租无发票单独退回，但如果合同/周期/金额/收款方未读到或不匹配，仍不得通过。

审批备注建议写清：常规周期性运营付款、合同/历史周期匹配、金额和收款方匹配；如无发票，补充“财务按发票、完税证明和税务入账口径留档”。
...
自动同意仅限“低风险 + 高确信度”且未被当前任务边界禁止的审批。自动同意也必须写明审批意见，并在执行后回读审批记录或待办列表确认结果。
