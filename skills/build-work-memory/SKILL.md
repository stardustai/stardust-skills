...
description: Guided first-run workflow for importing a user's existing work context into Friday Memory from local files, DingTalk documents/wiki, DingTalk AI minutes, and selected DingTalk data through dws. Use when the user wants to initialize Memory, connect their work context, import DingTalk knowledge, seed Memory from documents or meetings, or prepare Memory for daily work.
---
...
Run with only Codex, Friday Memory MCP, and `dws` DingTalk Workspace CLI. Do not require the `memory-connector` repository, Neo4j, Friday admin UI, local backend services, or project-local scripts. Use local file access only when the user explicitly chooses local documents.
...
If `dws` is missing, point the user to `https://github.com/DingTalk-Real-AI/dingtalk-workspace-cli` and ask them to install and log in. Verify with:
...
Do not ask the user for a project or topic. This skill is for first-time work-context initialization.

Do not default-scan anything. Only import ranges the user explicitly describes.

Do not require per-item confirmation. Collect scope, summarize the planned read range, then proceed unless the user interrupts.
...
每一步我都会问你想导入的范围。收集完范围后，我会在正式读取前汇总一次；如果范围不对，你可以打断我。不会默认扫描你的全部资料。
```
...
接下来我会开始读取这些范围内的内容，并上传到 Memory。过程中如果范围不对，你可以随时打断我。
```

Then proceed without per-item confirmation.
