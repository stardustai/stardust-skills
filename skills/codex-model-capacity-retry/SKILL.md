...
Before retrying a capacity-failed completion, the watcher sends the normal `continue` prompt to the same task and waits five seconds. It reads the latest `thread_goal_updated.goal.status` from that session's trajectory and sends `/goal resume` when the latest status is `active` or `paused`, so the existing Goal is reactivated after the interrupted turn. A task with no Goal, a completed Goal, or a Goal stopped by budget receives only `continue`; the watcher never invents a Goal or sends a Goal command to those tasks. The state file records the successful first `continue` per failure signature, so a temporary failure before the Goal step does not repeat `continue` or resume the Goal early.
...
If normal resume, or the conditional Goal resume for an active or paused Goal, returns `thread-store conflict` or `already has an active writer`, the watcher queues that exact prompt on the exact same thread with `codex queue --thread SESSION_ID --message PROMPT`. A successful `continue` queue is recorded as the first stage; a successful Goal queue records the failure signature, so the watcher does not enqueue duplicates every five seconds. If queueing fails, the signature remains retryable and the watcher tries again later. An incidental `401`/`token_revoked` warning does not override this active-writer result; a genuine authentication error with no writer conflict is suppressed as before.
...
The test proves capacity matching, same-session `continue`, reactivation of active and paused Goals, suppression of Goal commands for sessions without a Goal, no model/effort override, duplicate suppression, repeated retry after a new capacity failure, suppression of normal answer text that merely contains a `429` amount, historical rollout suppression, controller-session exclusion, and active-writer recovery by queueing exactly one same-thread prompt.
