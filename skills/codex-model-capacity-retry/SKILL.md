...
Before retrying a capacity-failed completion, the watcher sends the normal `continue` prompt to the same task, then waits five seconds and sends `/goal resume`. The state file records the successful first `continue` per failure signature, so a temporary failure on the second step does not repeat `continue` or resume the Goal early. If the task has no Goal, `/goal resume` is harmless. If the Goal was cleared, completed, or stopped by budget, the watcher cannot recreate it; the task needs a new Goal.
...
If either normal resume or Goal resume returns `thread-store conflict` or `already has an active writer`, the watcher queues the exact same prompt on the exact same thread with `codex queue --thread SESSION_ID --message PROMPT`. A successful `continue` queue is recorded as the first stage; a successful Goal queue records the failure signature, so the watcher does not enqueue duplicates every five seconds. If queueing fails, the signature remains retryable and the watcher tries again later. An incidental `401`/`token_revoked` warning does not override this active-writer result; a genuine authentication error with no writer conflict is suppressed as before.
...
The test proves capacity matching, same-session `continue` followed by Goal resume, no model/effort override, duplicate suppression, repeated retry after a new capacity failure, suppression of normal answer text that merely contains a `429` amount, historical rollout suppression, controller-session exclusion, and active-writer recovery by queueing exactly one same-thread `continue` and one same-thread Goal resume.
