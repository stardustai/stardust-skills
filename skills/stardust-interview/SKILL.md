...
6. 设置面试后自动复盘定时任务：面试准备完成后，创建一个**2 小时后的延后定时任务**，任务内容是按本 skill 自动拉取该候选人目标轮次的小青资料和 DWS AI 听记，若听记已生成则进行面评分析，并在满足提交条件时自动提交小青面评。任务标题/描述中必须包含候选人姓名、岗位、轮次和小青 `interview_id`，便于后续取消或排查。
