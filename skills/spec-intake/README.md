...
8. 市场验证阶段必须判断市场体量：客户价值、客户支付意愿、客单价、客户数量和市场竞争程度，不能只写一句“市场很大”；体量判断必须有证据来源，例如咨询/行业报告和一手客户反馈。
9. 产品论证阶段必须判断技术领先性和独特性：产品 owner 给出论述，spec agent 打分，产品 owner 确认。
10. 技术方案设计阶段必须读取源代码：spec agent 根据技术方案和代码事实打分，AI 工程师确认。
11. 用 `stage_gate` 控制接力，区分产品交接、技术缺口评审、验证方案设计、验证执行和工程排期。
12. 开场必须展示完整流程图，并用 Codex 进度工具或文本进度提示当前阶段。
13. 阶段切换必须先给退出总结并得到用户确认，不能静默从业务验证跳到产品论证。
14. 有 UI 的需求在 `product_ready` 或更后门禁前必须产出 SVG 线框图并让用户确认；业务验证/交接阶段可先标记为 `needed`。
15. 如果是 Domain Pack，必须明确 Workspace Memory、Task、Artifact、Recipe、Feedback/Comment、Room 的闭环。
16. Memory 写入、自学习、隐私、人工确认和回滚必须结构化。
17. 验收测试统一进入 `validation_plan`，区分 `validation_design_ready` 和 `validation_execution_ready`。
18. 业务端到端成功流程统一进入顶层 `business_success_scenarios`：业务定义真实流程和成功终态，产品或 spec agent 负责结构化，业务 owner 负责确认。
19. QA 和工程不得重写业务成功定义；它们通过 `validation_plan.scenario_coverage` 补充 QA 用例、自动化计划、Eval 资产和执行证据。
20. 交付风险是产品输入：先采集用户范围、数据敏感度、写入影响、集成与权限、可恢复性、业务影响六个事实，再按最高风险维度确定 R0-R3，由 decision owner 确认。
21. 产品阶段必须把业务目标细化为产品目标、业务指标、用户旅程和用户操作流，方便 QA 生成测试用例，也方便工程埋点和 loop engineering。
22. 如果 spec 设计过程上下文很复杂，应提醒用户使用 subagent 分工推进，避免上下文过长导致遗忘；如果设计判断需要多视角评审，应虚拟 PM、算法、用户、专家、researcher、owner 等角色互相挑战假设，并写入 `review_gates.virtual_review_panel`；主 agent 仍负责 stage gate、冲突判断和最终 spec 综合。
...
命名规则：`customer POC` 指面向客户的商业试点，包含设计伙伴、预算 owner、客户 reviewer、客户数据、验收方式和付费/采购信号；`validation_design` 和 `validation_execution` 指产研内部验证阶段，不再用 PoC 命名。
...
```text
我建议从 business_feasibility 切到 product_shape。已确认 X；仍未确认 Y；因此只允许产品收敛，不允许客户 POC、内部验证或工程。是否确认结束业务验证？
```

## 上下文很长时的处理

复杂 spec 不适合只靠一个长对话硬撑。出现以下情况时，agent 应该主动提醒可以使用 subagent：

- 需要同时处理商业证据、竞品调研、市场报告、客户访谈和销售 pipeline。
- 需要同时展开产品目标、用户旅程、操作流、UI 线框图、Domain Pack 对象和 Memory 策略。
- 技术缺口评审要读多个仓库、多个文档或多条代码路径。
- QA 要把多个业务成功场景和操作流映射成测试覆盖。
- 对话已经很长，早期确认的事实、blocker 或 stage-exit 决策容易被遗忘。

推荐分工：

- Evidence subagent：整理市场、客户、竞品和设计伙伴证据。
- Product subagent：展开 workflow、user journey、operation flow、对象模型和线框图需求。
- Engineering subagent：读取代码和文档，判断能力边界和缺口。
- QA subagent：把业务场景和操作流映射到 QA 覆盖、指标和测试资产。

主 agent 不应把最终判断外包。它必须保留 stage gate、核验证据、处理冲突，并产出最终 spec。

虚拟评审团和 subagent 不是一回事。subagent 负责分头收集事实或展开工作，虚拟评审团负责在设计阶段提出不同视角的质疑。复杂需求可以虚拟出 PM、算法/ML、真实或代表性用户、领域专家、researcher、业务 owner、QA、合规、销售/GTM、工程等角色；每个角色要说明评审重点、挑战问题、评审发现和对决策的影响。如果角色之间意见冲突，先让它们互相评审假设，再由主 agent 收敛结论，记录到 `review_gates.virtual_review_panel`。

```text
一句话需求
└── 产品形态判断
    └── 业务验证 opportunity_assessment
        ├── commercial_context：客户、买方、付费原因、客户 POC、风险、销售动作
        ├── market_sizing：客户价值、支付意愿、客单价、客户数量、竞争强度、市场吸引力
...
├── scripts/
│   └── validate_spec.py - JSON 校验脚本，包含 stage gate、证据、UI、客户 POC、内部验证和 Memory 规则。
├── examples/
...
- PMF 分数 >= 3 是否有非假设证据支撑。
- 业务验证是否包含市场体量：客户价值、支付意愿、客单价、客户数量、竞争强度、市场吸引力、证据质量和证据置信度。
...
- `validation_plan` 是否区分验证方案设计 ready 和验证执行 ready。
...
| `opportunity_assessment.market_sizing` | 业务验证 | 业务 owner / PMF owner / 销售 | 分层判断市场体量和商业吸引力：客户价值、支付意愿、客单价、可触达客户数量、竞争强度、总体吸引力、证据质量和证据置信度。 | 不替代 PMF，也不替代竞品矩阵；它回答机会大小和是否值得投入。不能只靠内部估算。 |
...
| `validation_plan` | QA / 验证方案设计 | QA owner / 产品 owner | 统一管理验收、指标、测试资产和 `scenario_coverage`，把业务成功场景映射为 QA 与工程验证，并区分验证方案设计 ready 和验证执行 ready。 | 不重新定义业务成功终态，也不做运维监控；监控在 `operation_standards`。 |
| `operation_standards` | 验证执行 / 交付准备 | DevOps owner / 工程 owner | 定义监控、兜底、支持和审计事件。 | 不定义 pass/fail 规则；规则在 `validation_plan.metrics`。 |
...
| `review_gates` | 阶段评审 | 各 owner / 虚拟评审团 | 记录业务、产品、AI 工程、QA、合规、工程启动前分别要 review 什么；复杂设计还要记录 PM、算法、用户、专家、researcher、owner 等虚拟角色如何互相挑战假设并影响决策。 | 不替代 `stage_gate` 的当前决策；虚拟角色不能替代真实 owner 确认。 |
...
- `market_sizing` 放在 `opportunity_assessment` 下，因为它属于 business 阶段的机会体量判断，不属于产品形态或技术方案。
...
├── status: enum - 当前状态，例如 draft、spec_review、validation_design、ready_for_engineering。
├── priority: enum - 业务或产研优先级，例如 P0、P1、P2、P3、unknown。
│
├── stage_gate: object - 当前能走到哪一关，以及明确不能走到哪一关。
│   ├── current_stage: enum - 当前阶段：业务验证、产品形态、技术缺口评审、验证方案设计、验证执行、工程交付。
│   ├── readiness_label: enum - 当前准备度：not_ready、business_ready、product_ready、engineering_gap_review_ready、validation_design_ready、validation_execution_ready、engineering_ready、review_required。
...
│   ├── commercial_context: object - 客户类型、L2/L3、买方、客户池、付费原因、客户 POC 路径、价格假设、交付风险、销售动作。
│   ├── market_sizing: object - 市场体量和商业吸引力，包含客户价值、支付意愿、客单价、客户数量、竞争强度、市场吸引力、证据质量和证据置信度。
...
│
├── validation_plan: object - 业务验收、产品验收、内部验证准备度、指标和测试资产。
...
│   ├── validation_design_ready: object - 验证方案设计是否 ready。
│   ├── validation_execution_ready: object - 验证执行是否 ready。
...
├── review_gates: object - 业务、产品、AI 工程、QA、合规、工程启动前和虚拟多角色评审的内容。
│   └── virtual_review_panel: array - PM、算法、用户、专家、researcher、owner 等虚拟角色的评审重点、挑战问题、发现和决策影响。
